...
SET search_path = 'igrc';                      --  !REPLACE schema_name
\set SchemaVariable '''igrc'''                 --  !REPLACE schema_name 
...
\echo ===============================================
\echo Average Size of one timeslot in portal tables :
\echo 

SELECT 'Average Size of one timeslot in tportal tables (Total Space MB) : ', pg_size_pretty(SUM(pg_relation_size(relid))/(SELECT COUNT(1) from timportlog where cportal='1')) FROM pg_catalog.pg_statio_user_tables
   WHERE relname like 'tportal%';


-- #Number of index per table
\echo ===================================
\echo Number of index per table
\echo

SELECT
    count(indexname),tablename
FROM
    pg_indexes
WHERE
...
\echo ===================================
\echo List of indexes by table 
\echo 

SELECT
    PGI.tablename as TableName,
    PGI.indexname as IndexName,
    PGI.indexdef as IndexDefinition
FROM pg_indexes AS PGI
WHERE
    PGI.schemaname = :SchemaVariable
ORDER BY
    tablename,
    indexname;
...
--
-- FYI : You can kill a blocked query by using the below command.
--
...
SELECT
	...
	pg_size_pretty(pg_relation_size(PSUI.indexrelid::regclass)) AS IndexSize,
